...
// ############################################################################
let responseid = IdentityUpdate("did:leaf:e60da46839cca10dc2625b559c1387b071a76341bc4ebc480bbcb7d0c6b6d081", "sdcxshsss", "RefugeeIdentification", "UnitedNations", "10/31/2022");
...
function IdentitySearch(values) {
    ...
    asyncresponses = "";
    var resp = postRequest(APIURL + "/findValueHashes", reqdoc, reqdoc.searchvalue);
    ...
    for (let i = 0; i < hashlist.length; i++) {
        ...
        var resp = postRequest(APIURL + "/searchsalteddocs", reqdoc, hashlist[i].hash)
        while (asyncresponses == "") {
            sleep(100);
        }
    ...
    for (let i = 0; i < saltids.length; i++) {
        ...
        asyncresponses = "";
        var resp = postRequest(APIURL + "/searchDocuments", reqdoc, saltids[i].docid)
        while (asyncresponses == "") {
            sleep(100);
        }
...
// document upload.
// ####################################################################################
function IdentityUpdate(did, qrCode, documentType, documentIssuer, issuanceDate) {
    ...
    var signature = forge.util.bytesToHex(forge.ed25519.sign({ message: shaval, encoding: 'utf8', privateKey: idpair.privateKey }));
...
function postRequest(url, reqdoc, hashValue) {
    ...
    var signature = forge.util.bytesToHex(forge.ed25519.sign({ message: hashValue, encoding: 'utf8', privateKey: idpair.privateKey }));
